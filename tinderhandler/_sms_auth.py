...
def send_code(number):
    ...
    print(r.url)
    response = r.json()
    if not response.get("login_request_code"):
        return False
    else:
        return response["login_request_code"]


def get_token(number, code, req_code):
    ...
    gettoken_content = json.dumps(
        {"token": access_token, "id": access_id, "client_version": "9.0.1"}
    )
    gettoken_headers = {
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 11_2_5 like Mac OS X) AppleWebKit/604.5.6 (KHTML, like Gecko) Mobile/15D60 AKiOSSDK/4.29.0",
        "Content-Type": "application/json",
    }
    resp_gettoken = requests.post(
        TOKEN_URL, data=gettoken_content, headers=gettoken_headers, verify=False
    )
    token_response = resp_gettoken.json()
    if not token_response["data"].get("api_token"):
...
def get_token_sms():
    phone_number = input(
        "Please enter your phone number under the international format (country code + number):\n"
    )
    log_code = send_code(phone_number)
    sms_code = input("Please enter the code you've received by sms:\n")
    token = str(get_token(phone_number, sms_code, log_code))
    return token
